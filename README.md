...
$parser->fromFile('file.csv', [
    'columns' => [
        ...
        // complete usage
        'col2' => function ($data, $index, array $row, array $parsed, array $meta, array $options) {
            return $data;
        }
    ]
]);
...
$parser->fromFile('file.csv', [
    ...
    // complete ussage
    'onBeforeColumnParse' => function ($data, $index, array $meta, array $options)
    {
        return utf8_encode($data);
    }
]);
...
$result = $parser->fromString($str, [
    'columns' => [
        '0 as id' => function ($data) {
            return (int) $data;
        },
        '1 as content' => function ($data) {
            return ucfirst($data);
        }
    ]
]);
